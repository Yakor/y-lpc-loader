  ...
  MACRO_YAML_DOCUMENT(file_name,
                      {
                        ...
                        MACRO_YAML_SEQUENCE(CONFIG_EXECUTABLES,i,
                                            ...
                                            {
                                              MACRO_YAML_STRING(CONFIG_PRIMARY_FILENAME,config->executables[i].primary_filename);
                                              MACRO_YAML_INT(CONFIG_IRAM_ADDRESS,config->executables[i].iram_address,{});
                                              MACRO_YAML_STRING(CONFIG_SECONDARY_FILENAME,config->executables[i].secondary_filename);
                                              MACRO_YAML_INT(CONFIG_SDRAM_ADDRESS,config->executables[i].sdram_address,{});
                                            }
                                            );
                      }
                      );
